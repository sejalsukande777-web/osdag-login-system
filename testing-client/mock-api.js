(function () {
  ...
  function loadSeed(seed) {
    for (const u of seed.users) {
      const record = { id: u.id, email: u.email, passwordHash: fakeHash(u.password), profile: u.profile };
  ...
  async function init() {
    try {
      ...
      console.warn("[mock-api] could not load seed-data.json (" + err.message + "), using built-in fallback data.");
  ...
  function authenticate(req) {
    ...
    if (Date.now() > session.expiresAt) { state.sessions.delete(token); return null; }
  ...
  async function handleRegister(req) {
    ...
    const record = { id, email, passwordHash: fakeHash(password), profile: { fullName: "", displayName: email.split("@")[0], bio: "", createdAt: new Date().toISOString(), role: "user" } };
  ...
  async function handleLogin(req) {
    ...
    if (lock && lock.lockedUntil && Date.now() < lock.lockedUntil) return json(429, { error: "Too many failed attempts. Try again in a bit." });
    ...
    if (!valid) {
      ...
      entry.count += 1;
      if (entry.count >= MAX_FAILED_ATTEMPTS) { entry.lockedUntil = Date.now() + LOCKOUT_MS; entry.count = 0; }
