...
- Real logout — this one actually surprised me. In my custom backend I had to build a whole `revoked_tokens` table and check it on every request just to make logout actually kill a JWT. In Appwrite, `account.deleteSession("current")` just works — I tested it and confirmed the same session is immediately rejected (401) on the next request, with zero extra code from me.
